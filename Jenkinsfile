pipeline {
    ...
    stages {
        ...
        stage('SCM') {
            steps {
               git branch: 'master', url: 'https://github.com/Stywar/TRACIS-CI-NOV22'
            }
        }
...
stage ("Deploy AKS") {
           steps {
            bat(script: 'az aks get-credentials --resource-group Devops --name devopsdec22  & kubectl config get-contexts --kubeconfig=%KUBE_PATH_CONFIG%', returnStdout: true);
            bat(script: 'kubectl config use-context devopsdec22  --kubeconfig=%KUBE_PATH_CONFIG%', returnStdout: true);
